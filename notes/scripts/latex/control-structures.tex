...
\begin{frame}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{pics/introduction/conditional.png}
...
\begin{itemize}
    \item The accumulator pattern is a common programming pattern that involves accumulating a result over multiple iterations.
    \item It is often used with loops to calculate a sum, product, or other aggregate value.
    \item The accumulator pattern typically involves initializing an accumulator variable to an initial value and updating it in each iteration of the loop.
\end{itemize}
 \begin{lstlisting}[language=Python]
total = 0
for i in range(1, 6):
